...
class CWebPConverter;

int ProgressReport(int percent, const WebPPicture* const picture);


class CWebPConverter : public FileConverter {
public:
    CWebPConverter( IzInstanceBase *instance,const pp::Var& var_message) : FileConverter(instance, var_message){
    };
    ...
    static std::string baseParameters(uint64_t taskId, std::string inputFullPath, uint64_t inputSize, std::string outputDirectoryPath, std::string baseName, std::string inputExtension){
        return "\\\""+inputFullPath+"\\\" -o \\\""+outputDirectoryPath+baseName+".webp\\\"";
    }
...
private:
    ...
    int main(int argc,char* argv[]) {
        //int argc=5;
        //const char *argv[] ={"./cwebp",inputFullPath.c_str(),"-o",(directoryPath+baseName+".webp").c_str(),"-progress"};
        //char* out_file = (char*)malloc(256);
        int short_output = 1;
  ...
  int use_lossless_preset = -1;  // -1=unset, 0=don't use, 1=use it
        ...
        for (c = 1; c < argc; ++c) {
            ...
            if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
                ...
                out_file = argv[++c];
                /*std::string s(argv[++c]);
                memcpy(out_file, s.c_str(), s.length());*/
                //instance_->DebugMessage("out="+std::string(argv[c]));
               // instance_->DebugMessage("out_file="+std::string(out_file));
      ...
      if (use_lossless_preset != 0) use_lossless_preset = 1;
                ...
                if (!strcmp(argv[c], "none")) {
                    ...
                    UpdateTaskDetails("Error! Unrecognized alpha filter:"+std::string(argv[c]));
                    goto Error;
                }
                ...
                if (!strcmp(argv[c], "photo")) {
                    ...
                    UpdateTaskStatus(ERRORED);
                    UpdateTaskDetails("Error! Unrecognized image hint:"+std::string(argv[c]));
                    goto Error;
                }
                ...
                if (!strcmp(argv[c], "default")) {
                    ...
                    UpdateTaskStatus(ERRORED);
                    UpdateTaskDetails("Error! Unrecognized preset:"+std::string(argv[c]));
                    goto Error;
                }
                if (!WebPConfigPreset(&config, preset, config.quality)) {
                    UpdateTaskStatus(ERRORED);
                    UpdateTaskDetails("Error! Could initialize configuration with preset.");
                    goto Error;
                }
                ...
                while (start < end) {
                    ...
                    if (i == kNumTokens) {
                        UpdateTaskStatus(ERRORED);
                        char error[256];
                        sprintf(error, "Error! Unknown metadata type '%.*s'\n",
                                (int)(token - start), start);
                        UpdateTaskDetails(error);
                        HelpLong();
                        return -1;
                    }
                    start = token + 1;
      ...
      if (keep_metadata != 0 && keep_metadata != METADATA_ICC) {
        ...
        fprintf(stderr, "Warning: only ICC profile extraction is currently"
                ...
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "Error! Unknown option '%s'\n", argv[c]);
                UpdateTaskDetails(error);
        ...
        if (in_file == NULL) {
            UpdateTaskStatus(ERRORED);
            char error[256];
            sprintf(error, "No input file specified!\n");
            UpdateTaskDetails(error);
            HelpShort();
            goto Error;
        }
      ...
      goto Error;
        ...
        if (!quiet && config.lossless == 1) {
            if (config.target_size > 0 || config.target_PSNR > 0) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "Encoding for specified size or PSNR is not supported"
            ...
            if (config.partition_limit > 0) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "Partition limit option is not required for lossless"
        ...
        if (!WebPValidateConfig(&config)) {
            UpdateTaskStatus(ERRORED);
            char error[256];
            sprintf(error, "Error! Invalid configuration.\n");
            UpdateTaskDetails(error);
            goto Error;
        }
        ...
        if (!ReadPicture(in_file, &picture, keep_alpha,
            ...
            UpdateTaskStatus(ERRORED);
            char error[256];
            sprintf(error, "Error! Cannot read input picture file '%s'\n", in_file);
            UpdateTaskDetails(error);
            goto Error;
        }
        ...
        if (verbose) {
            ...
            UpdateTaskStatus(ERRORED);
            char error[256];
            sprintf(error, "Time to read input: %.3fs\n", read_time);
            UpdateTaskDetails(error);
        }
        ...
        if (out_file != NULL) {
            ...
            if (out == NULL) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error,"Error! Cannot open output file '%s'\n", out_file);
                UpdateTaskDetails(error);
                goto Error;
            } else {
            ...
            if (!quiet && !short_output) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "No output file specified (no -o flag). Encoding will\n be performed, but its results discarded.\n\n");
                UpdateTaskDetails(error);
            }
        }
        if (!quiet) {
            picture.stats = &stats;
            picture.user_data = malloc(sizeof(uint64_t));
            *((uint64_t*)picture.user_data) = id_;
        }
        ...
        if (crop != 0) {
            ...
            if (!WebPPictureView(&picture, crop_x, crop_y, crop_w, crop_h, &picture)) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "Error! Cannot crop picture\n");
                UpdateTaskDetails(error);
                goto Error;
            }
        }
        if ((resize_w | resize_h) > 0) {
            if (!WebPPictureRescale(&picture, resize_w, resize_h)) {
                UpdateTaskStatus(ERRORED);
                char error[256];
                sprintf(error, "Error! Cannot resize picture\n");
                UpdateTaskDetails(error);
                goto Error;
            }
        }
        ...
        if (!WebPEncode(&config, &picture)) {
            UpdateTaskStatus(ERRORED);
            char error[256];
            sprintf(error,"Error! Cannot encode picture as WebP\n Error code: %d (%s)\n",
        ...
        if (verbose) {
            ...
            printf("Time to encode picture: %.3fs\n", encode_time);
        }
        ...
        if (dump_file) {
            if (picture.use_argb) {
                char desc[256];
                sprintf(desc, "Warning: can't dump file (-d option) in lossless mode.");
                instance_->DebugMessage(desc);
        ...
        if (keep_metadata != 0) {
            if (out != NULL) {
                if (!WriteWebPWithMetadata(out, &picture, &memory_writer,
                    ...
                    UpdateTaskStatus(ERRORED);
                    char error[256];
                    sprintf(error,"Error writing WebP file with metadata!\n");
                    UpdateTaskDetails(error);
                    goto Error;
                }
        ...
        if (!quiet) {
            ...
            if (print_distortion >= 0) {    // print distortion
                ...
                if (original_picture.use_argb &&
                    ...
                    UpdateTaskStatus(ERRORED);
                    char error[256];
                    sprintf(error,"Error while converting original picture to YUVA.\n");
                    UpdateTaskDetails(error);
                    goto Error;
                }
                if (picture.use_argb &&
                    ...
                    UpdateTaskStatus(ERRORED);
                    char error[256];
                    sprintf(error,"Error while converting compressed picture to YUVA.\n");
                    UpdateTaskDetails(error);
                    goto Error;
                }
                if (!WebPPictureDistortion(&picture, &original_picture,
                    ...
                    UpdateTaskStatus(ERRORED);
                    char error[256];
                    sprintf(error,"Error while computing the distortion.\n");
                    UpdateTaskDetails(error);
                    goto Error;
                }
                if (!short_output) {
                    char desc[256];
                    sprintf(desc, "%s: Y:%.2f U:%.2f V:%.2f A:%.2f  Total:%.2f\n",
...
int ProgressReport(int percent, const WebPPicture* const picture) {
    IzInstance<CWebPConverter>::current->getTask(*((uint64_t*)picture->user_data))->UpdateProgress((int8_t)percent);
    return 1;  // all ok
}


class CWebPModule : public pp::Module {
public:
    CWebPModule() : pp::Module() {}
    virtual ~CWebPModule() {}

    virtual pp::Instance* CreateInstance(PP_Instance instance) {
        return new IzInstance<CWebPConverter>(instance);
    }
};

namespace pp {
    Module* CreateModule() {
        return new CWebPModule();
    }
}
