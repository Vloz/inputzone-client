...
class DWebPConverter : public FileConverter {
public:
    DWebPConverter(IzInstanceBase *instance, const pp::Var &var_message) : FileConverter(instance, var_message) {
    };
    ...
    static std::string baseParameters(uint64_t taskId, std::string inputFullPath, uint64_t inputSize, std::string outputDirectoryPath, std::string baseName, std::string inputExtension) {
        return "\\\"" + inputFullPath + "\\\" -o \\\"" + outputDirectoryPath + baseName + "%OUTPUT_EXT%\\\"";
    }
...
public:
    ...
    virtual pp::Instance* CreateInstance(PP_Instance instance) {
        return new IzInstance<DWebPConverter>(instance);
    }
};
