...
std::string createPrerunRequest(char *data, int size){
    ...
    uint64_t inputSize = getMessageSize(message);
    uint64_t outputSize = estimateOutputSize(inputSize);
    std::string inputPath = getMessageInputFullPath(message);
    std::string baseParams = baseParameters(getMessageFullId(message),inputPath,inputSize, getMessageOutputDirectoryPath(message)+"/", basename(inputPath), getExtension(inputPath));
    std::string answer = "{\"estitmateSize\":\""+std::to_string(outputSize)+"\",\"baseParameters\":\""+baseParams+"\"}";
    return answer;
}
...
void iz_release(IZ_FILE* output){
    ...
    std::string inputPath = (_taskProps.mountPoint+ getMessageInputFullPath(_taskProps.startMessage)).c_str();
...
void onDownloadInputFinished(unsigned handle, void* args, const char* fullpath){
    ...
    char* argv[256];
    int argc = getMessageParams(_taskProps.startMessage, argv);
    (*_taskProps.main_func)(argc,argv);
}
...
void iz_init(char *data, int size, iz_main_func mainCallback,iz_postMessage_func postMessageFunc, iz_postFinalMessage_func postFinalMessageFunc) {
    ...
    _taskProps.main_func = mainCallback;
    ...
    if(currentBrowser== CHROME){
        ...
        (_taskProps.mountPoint + getMessageDirectoryPath(data)).c_str(), (_taskProps.mountPoint + getMessageInputDirectoryPath(data)).c_str(),
        (_taskProps.mountPoint + getMessageOutputDirectoryPath(data)).c_str(),(int)currentFsType);
        ...
        initFS = EM_ASM_INT(
                {
                    try{
                        ...
                        FS.mkdir(Pointer_stringify($2)+'/InputZone');
    ...
    if(currentBrowser == CHROME ){
        ...
        iz_updatePreProgress((uint8_t)0);
        char* argv[256];
        int argc = getMessageParams(data, argv);
        (*_taskProps.main_func)(argc,argv);
    }
...
void iz_console(std::string message){
    postMessage(consoleMsg(_taskProps.id, message));
}
